...
Private Sub lstProducts_Click()
    ...
    txtUnitMeasure = GetUnitMeasureFromCode(lstProducts.Column(3))
...
Private Sub cmdAdd_Click()
    ...
    With frmInvoice.lstItems
        ...
        .List(p, 3) = txtTotal
        .List(p, 4) = txtCode
        .List(p, 5) = GetUnitMeasureFromName(txtUnitMeasure)
    End With
...
Private Function ValidFields() As Boolean
    ...
    If Trim(txtCode) = Empty Then
        MsgBox "El producto no tiene un código. Revise la base de datos de productos.", vbExclamation, "Subsane la observación"
        Exit Function
    End If
    If Trim(txtUnitMeasure) = Empty Then
        MsgBox "El producto no tiene una unidad de medida. Revise la base de datos de productos.", vbExclamation, "Subsane la observación"
        Exit Function
    End If
    ...
    If Trim(txtTotal) = Empty Or Not IsNumeric(Trim(txtTotal)) Then
        MsgBox "El precio total presenta inconsistencias. Debe ser un número mayor a cero.", vbExclamation, "Subsane la observación"
        txtTotal.SetFocus
        Exit Function
    End If
